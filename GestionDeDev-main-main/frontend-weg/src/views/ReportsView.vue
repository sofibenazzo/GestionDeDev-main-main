...
const fetchData = async () => {
  try {
    const response = await api.get('/api/Remito'); // ✅ CORREGIDO: era '/Remito'
...
const stats = computed(() => {
  ...
  return {
    ...
    pending,
    inReview,
    // ✅ CORREGIDO: evitar división por cero cuando no hay datos
    acceptedRate: total > 0 ? Math.round((accepted / total) * 100) : 0,
    rejectedRate: total > 0 ? Math.round((rejected / total) * 100) : 0,
    pendingRate: total > 0 ? Math.round(((pending + inReview) / total) * 100) : 0,
  };
});
          ...
          <div class="space-y-6">
            <div class="space-y-2">
              ...
              <div class="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                <!-- ✅ CORREGIDO: usar stats computado en vez de división directa con posible 0 -->
                <div class="h-full bg-emerald-500 transition-all duration-1000" :style="{ width: stats.acceptedRate + '%' }"></div>
              </div>
            </div>
            <div class="space-y-2">
              <div class="flex justify-between text-xs font-bold uppercase tracking-wider text-slate-500">
              ...
              <div class="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                <div class="h-full bg-rose-500 transition-all duration-1000" :style="{ width: stats.rejectedRate + '%' }"></div>
              </div>
            </div>
            <div class="space-y-2">
              <div class="flex justify-between text-xs font-bold uppercase tracking-wider text-slate-500">
              ...
              <div class="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                <div class="h-full bg-amber-500 transition-all duration-1000" :style="{ width: stats.pendingRate + '%' }"></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
          ...
          <div class="h-48 flex items-end gap-3 px-4">
            <div v-for="i in 12" :key="i" class="flex-1 bg-slate-50 rounded-t-xl group relative cursor-pointer hover:bg-blue-50 transition-all border-b-2 border-slate-100">
              <div
                class="absolute bottom-0 left-0 right-0 bg-[#005792] rounded-t-xl transition-all duration-1000 group-hover:opacity-80"
