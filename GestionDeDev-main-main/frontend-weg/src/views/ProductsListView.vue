...
import { Plus, Search, Package, Edit, Trash2, Loader2 } from 'lucide-vue-next';
...
const fetchProducts = async () => {
  try {
    const response = await api.get('/api/Producto'); // ✅ CORREGIDO: era '/Producto'
...
const deleteProduct = async (id: number) => {
  if (confirm('¿Está seguro de que desea eliminar este producto?')) {
    try {
      ...
      products.value = products.value.filter(p => p.productoId !== id);
    } catch (error) {
...
// ✅ CORREGIDO: era una función normal, ahora es computed para reactividad correcta
  ...
  <div class="flex min-h-screen bg-[#fcfcfd]">
    ...
    <main class="flex-1 p-8 max-w-7xl mx-auto overflow-y-auto no-scrollbar animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div>
          <h1 class="text-4xl font-black tracking-tighter text-slate-900 mb-2">Fichas Técnicas</h1>
          <p class="text-slate-500 font-medium">Control de trazabilidad y especificación de mercadería — WEG.</p>
        </div>
        <button @click="router.push('/products/new')" class="flex items-center gap-2 bg-[#005792] text-white px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-[#004a7c] transition-all shadow-xl shadow-blue-900/20 active:scale-95">
          <Plus class="h-5 w-5" />
          Nuevo Producto
        </button>
      </div>
        ...
        >
          <div class="flex items-start justify-between mb-6">
            ...
            <div class="flex gap-2">
              <button @click="router.push(`/products/edit/${product.productoId}`)" class="p-2 text-slate-400 hover:text-[#005792] hover:bg-slate-50 rounded-xl transition-all">
                <Edit class="h-5 w-5" />
              </button>
              <button @click="deleteProduct(product.productoId)" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all">
      ...
      <div v-else class="flex flex-col items-center justify-center py-20 text-slate-300">
        ...
        <p class="text-xs font-black uppercase tracking-widest">No se encontraron productos</p>
      </div>
    </main>
  </div>
</template>